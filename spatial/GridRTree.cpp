...
RTree::Node * createTree(ItemGeoGrid & grid, uint32_t xmin, uint32_t ymin, uint32_t xmax, uint32_t ymax) {
	...
	if (xcount == 1 && ycount = 1) { //we have reached the end, create leafNode) {
		...
		in->leafNode = false;
		in->rect = grid.multiCellBoundary(xmin,ymin,xmax,ymax);
		if (xcount >= 2 && ycount >= 2) { //for children
			RTree::Node * node = createTree(xmin, ymin, xmin+xcount/2, ymin+ymin/2); //bottom left
			if (node)
				in->children.push_back(node);
			RTree::Node * node = createTree(xmin+xcount/2+1, ymin, xmax, ymin+ymin/2); //bottom right
			if (node)
				in->children.push_back(node);
			RTree::Node * node = createTree(xmin, ymin+ycount/2+1, xmin+xcount/2, ymax); //top left
			if (node)
				in->children.push_back(node);
			RTree::Node * node = createTree(xmin+xcount/2+1, ymin+ycount/2+1, xmax, ymax); //top right
			if (node)
				in->children.push_back(node);
		}
		else if (xcount >= 2) {
...
GridRTree::bulkLoad(const sserialize::spatial::GridRTree::BulkLoadType & idToRect) {
