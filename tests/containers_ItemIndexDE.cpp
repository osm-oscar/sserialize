...
class ItemIndexPrivateDETest: public CppUnit::TestFixture {
CPPUNIT_TEST_SUITE( ItemIndexPrivateDETest );
...
public:
	...
	void testRandomEquality() {
		...
		for(size_t i = 0; i < setCount; i++) {
			std::set<uint32_t> realValues( myCreateNumbers(rand() % setCount) );
			UByteArrayAdapter dest(new std::vector<uint8_t>(), true);
			ItemIndexPrivateDE::create(realValues, dest);
			ItemIndex idx(dest, ItemIndex::T_DE);
	...
	void testIntersect() {
		...
		ItemIndexPrivateDE::create(a, destA);
		ItemIndexPrivateDE::create(b, destB);
		ItemIndex idxA(destA, ItemIndex::T_DE);
		ItemIndex idxB(destB, ItemIndex::T_DE);
	...
	void testUnite() {
		...
		for(size_t i = 0; i < setCount; i++) {
			...
			UByteArrayAdapter destA(new std::vector<uint8_t>(), true);
			UByteArrayAdapter destB(new std::vector<uint8_t>(), true);
			ItemIndexPrivateDE::create(realValuesA, destA);
			ItemIndexPrivateDE::create(realValuesB, destB);
	...
	void testDynamicBitSet() {
		...
		for(size_t i = 0; i < setCount; i++) {
			...
			idx.putInto(bitSet);
			CPPUNIT_ASSERT_MESSAGE("index from DynamicBitSet unequal", idx == bitSet.toIndex(ItemIndex::T_DE));
...
int main() {
	...
	runner.addTest(  ItemIndexPrivateDETest::suite() );
	runner.run();
	return 0;
}