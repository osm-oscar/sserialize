...
class ItemIndexPrivateRleDETest: public CppUnit::TestFixture {
CPPUNIT_TEST_SUITE( ItemIndexPrivateRleDETest );
...
public:
	...
	void testIntersect() {
		for(uint32_t runs = 0; runs < 256; ++runs) {
			...
			ItemIndexPrivateRleDE::create(a, destA);
			ItemIndexPrivateRleDE::create(b, destB);
			ItemIndex idxA(destA, ItemIndex::T_RLE_DE);
			ItemIndex idxB(destB, ItemIndex::T_RLE_DE);
	...
	void testUnite() {
		...
		for(size_t i = 0; i < 256; i++) {
			...
			UByteArrayAdapter destA(new std::vector<uint8_t>(), true);
			UByteArrayAdapter destB(new std::vector<uint8_t>(), true);
			ItemIndexPrivateRleDE::create(realValuesA, destA);
			ItemIndexPrivateRleDE::create(realValuesB, destB);
...
int main() {
	...
	runner.addTest(  ItemIndexPrivateRleDETest::suite() );
	runner.run();
	return 0;
}