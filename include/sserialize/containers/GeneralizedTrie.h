...
template<class ItemType>
template<typename T_ITEM_FACTORY, typename T_ITEM>
bool GeneralizedTrie<ItemType>::staticFromStringsFactory(const T_ITEM_FACTORY & stringsFactory, UByteArrayAdapter & dest) {
	std::unordered_map<std::string, std::unordered_set<Node*> > strIdToSubStrNodes;
	std::unordered_map<std::string, std::unordered_set<Node*> > strIdToExactNodes;
	...
	//Now add the items
	ProgressInfo progressInfo;
	progressInfo.begin(stringsFactory.end()-stringsFactory.begin(), "GeneralizedTrie::fromStringsFactory::calculating storage need");
	uint32_t count = 0;
	for(typename T_ITEM_FACTORY::const_iterator itemIt(stringsFactory.begin()), itemEnd(stringsFactory.end()); itemIt != itemEnd; ++itemIt) {
		std::unordered_set<Node*> exactNodes;
		std::unordered_set<Node*> suffixNodes;
	...
	progressInfo.begin(stringsFactory.end()-stringsFactory.begin(), "GeneralizedTrie::fromStringsFactory::popluating exact/suffix index");
	uint32_t count = 0;
	for(typename T_ITEM_FACTORY::const_iterator itemIt(stringsFactory.begin()), itemEnd(stringsFactory.end()); itemIt != itemEnd; ++itemIt) {
		std::unordered_set<Node*> exactNodes;
		std::unordered_set<Node*> suffixNodes;
