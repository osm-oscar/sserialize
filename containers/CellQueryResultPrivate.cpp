...
CellQueryResult * CellQueryResult::intersect(const CellQueryResult * oPtr) const {
	...
	CellQueryResult * rPtr = new CellQueryResult();
	...
	for(uint32_t myI(0), myEnd(m_desc.size()), oI(0), oEnd(o.m_desc.size()); myI < myEnd && oI < oEnd;) {
		...
		if (myCD.cellId < oCD.cellId) {
			++myI;
			continue;
		}
		else if ( oCD.cellId < myCD.cellId) {
			++oI;
			continue;
		}
		...
		case 0x0: //both partial
			{
				...
				if (res.size()) {
					...
					r.m_desc.push_back(CellDesc(0, 1, myCD.cellId));
				}
			}
			break;
		case 0x1: //o full
			if (myCD.fetched) {
				...
				r.m_idx[m_desc.size()].idxPtr = m_idx[myI].idxPtr;
			}
			r.m_desc.push_back(myCD);
			break;
		case 0x2: //my full
			if (oCD.fetched) {
				r.uncheckedSet(r.m_desc.size(), o.m_idx[myI].idx);
			}
			else {
				r.m_idx[m_desc.size()].idxPtr = o.m_idx[myI].idxPtr;
			}
			r.m_desc.push_back(oCD);
			break;
		case 0x3:
			if (oCD.fetched) {
				r.uncheckedSet(m_desc.size(), idx(oI));
			}
			else if (myCD.fetched) {
				r.uncheckedSet(m_desc.size(), idx(myI));
			}
			r.m_desc.push_back(CellDesc(1, oCD.fetched | myCD.fetched, myCD.cellId));
			break;
		default:
			break;
		};
